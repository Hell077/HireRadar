...
- Docker verification covered API-run migration and idempotent restart, PostgreSQL/Redis readiness, RustFS bucket creation, Redis failure and recovery, and API shutdown. Milestone 1 is in progress: registration, Argon2id hashing, Ed25519 access tokens, refresh rotation, logout, email verification, password reset, and Redis-backed per-IP auth rate limits are implemented. Verification and reset requests write delivery events to the outbox; an email sender is still needed before milestone 1 is complete. Password reset revokes refresh sessions; previously issued 15-minute access tokens remain valid until expiry. Milestones 2–8 remain planned.
