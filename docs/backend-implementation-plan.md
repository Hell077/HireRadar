...
- Milestone 4 is in progress: migration 00007 adds source registry, sync run history, and immutable raw-job snapshots. A bounded `source-worker` claims due sources with `FOR UPDATE SKIP LOCKED`, isolates source failures, backs off failed sources, and stores raw job snapshots with content hashes and sync counters transactionally. Greenhouse, Lever, and Ashby public-board adapters map stable IDs and preserve upstream job JSON; enabled sources are listed at `GET /api/v1/sources`. Contract tests cover mapping, pagination, invalid boards, raw payloads, and HTTP failures. Sources are currently provisioned by inserting registry rows in PostgreSQL; admin CRUD, GitHub discovery, retry/replay operations, and worker/PostgreSQL integration coverage remain before this milestone can be marked complete.
...
Use `source_type` `greenhouse`, `lever`, or `ashby` and set `board` to that provider's board slug. Only add public job boards whose listings are intended for public display.
