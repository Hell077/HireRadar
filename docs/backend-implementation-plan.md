...
- Docker verification covered API-run migration and idempotent restart, PostgreSQL/Redis readiness, RustFS bucket creation, Redis failure and recovery, and API shutdown. Milestone 1 is in progress: registration, Argon2id hashing, Ed25519 access tokens, refresh rotation, logout, email verification, password reset, Redis-backed per-IP auth rate limits, and an SMTP outbox worker are implemented. The Docker stack routes email to Mailpit; the web verification, forgot-password, and reset-password pages now submit to the API. Registration and sign-in pages are still static, and automated PostgreSQL integration tests remain to be added before the milestone gate is complete. Password reset revokes refresh sessions; previously issued 15-minute access tokens remain valid until expiry. Milestones 2–8 remain planned.
