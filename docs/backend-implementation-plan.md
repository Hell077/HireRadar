...
- Milestone 3 upload storage is implemented: migration 00005 stores private PDF metadata; authenticated Huma routes list, create presigned uploads, verify completion, return short-lived downloads, and delete. The AWS SDK S3 adapter uses RustFS internal and browser-reachable endpoints. Completion checks object size, content type, and `%PDF-` signature before atomically setting `uploaded` and writing one `resume.uploaded` outbox event. The RustFS bucket initializer also configures local web CORS. Parsing, profile suggestions, end-to-end verification, and milestones 4–8 remain.
