...
- Docker verification covered API-run migration and idempotent restart, PostgreSQL/Redis readiness, RustFS bucket creation, Redis failure and recovery, and API shutdown. Milestone 1 is in progress: user registration, Argon2id hashing, Ed25519 access tokens, refresh token rotation, and logout are implemented. Email verification, password reset, and rate limits remain. Milestones 2–8 remain planned.
