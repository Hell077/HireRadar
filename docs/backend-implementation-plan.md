...
- Milestone 2 is in progress: migration 00003 adds profile, normalized skill, alias, desired-position, job-preference, and source-preference tables. The authenticated `GET` and `PUT /api/v1/profile` endpoints validate ownership from the Ed25519 access token and persist `profile.changed` in the same transaction as a changed profile. Skills, positions, preferences, and source-preference APIs are next.
