...
- Milestone 2 is in progress: migration 00003 adds profile, normalized skill, alias, desired-position, job-preference, and source-preference tables; migration 00004 seeds common skill aliases. Authenticated `GET` and `PUT /api/v1/profile` and `/api/v1/profile/skills` validate ownership from the Ed25519 access token. Changed profile fields or skill sets persist `profile.changed` in the same transaction. Equivalent skill aliases resolve to one canonical skill, and an unchanged replacement emits no event. Positions, preferences, and source-preference APIs are next.
