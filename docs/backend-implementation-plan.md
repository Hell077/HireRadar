...
- Milestone 4 is in progress: migrations 00007–00008 add the source registry, GitHub source type, sync run history, and immutable raw-job snapshots. A bounded `source-worker` claims due sources with `FOR UPDATE SKIP LOCKED`, isolates source failures, backs off failed sources, and stores raw job snapshots with content hashes and sync counters transactionally. Greenhouse, Lever, Ashby, and GitHub public-source adapters map stable IDs and preserve upstream job JSON; enabled sources are listed at `GET /api/v1/sources`. Contract tests cover mapping, pagination, pull-request filtering, invalid source config, raw payloads, and HTTP failures; PostgreSQL integration verifies snapshot versions, counters, and failure retry. Sources are currently provisioned by inserting registry rows in PostgreSQL; admin CRUD and retry/replay operations remain before this milestone can be marked complete.
